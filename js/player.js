(function() {
	...
	Player.prototype.moveRight = function() {
		// بررسی حرکت در حالت ایستاده
		if (this.vel[1] === 0 && this.standing) {
			if (this.crouching) {
				this.noWalk();
				return;
			}
			this.acc[0] = this.moveAcc;  // شتاب برای حرکت به راست
			this.left = false;
		} else {
			this.acc[0] = this.moveAcc;  // ادامه شتاب حتی در هنگام پرش
		}
	};
	
	Player.prototype.moveLeft = function() {
		if (this.vel[0] === 0 && this.standing) {
			if (this.crouching) {
				this.noWalk();
				return;
			}
			this.acc[0] = -this.moveAcc;  // شتاب برای حرکت به چپ
			this.left = true;
		} else {
			this.acc[0] = -this.moveAcc;  // ادامه شتاب حتی در هنگام پرش
		}
